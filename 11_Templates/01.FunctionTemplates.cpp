 ...
 *  typename T as a parameter; Max takes two T parameters, but they both must be of the same type.  
 *  Templates with different types are also allowed (see later examples).  T can take on any valid type; 
 *  in this example T takes on the types int, double and const char*.
 * 
 *  Note that "Red" and "Blue" are char arrays and the compiler designates their type as: const char*,
 *  which is a pointer type.  This turns out to be a problem because the template will compare the 
 *  pointers and not their contents.  Since const char* arrays are constant, the compiler stores these
 *  const items along with the program code in read only memory (text segment).
 *  
 ...
 *                                  ... (more code)
 *                      0x45828534: "Blue"  (start of constants)
 *                      0x45828539: "Red"
 *                                  ...
 *
